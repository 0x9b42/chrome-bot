...
async function sendMessage(chatId, text, replyToMessageId = null) {
  ...
  if (!text || text.trim() === "") {
    console.warn(
      `Attempted to send empty or whitespace-only message to ${chatId}. Sending fallback.`,
    );
    text = "(Pesan kosong)";
  }
  ...
  while (remainingText.length > 0) {
    ...
    if (!isFirstChunk && chunkToSend.trim()) {
      ...
      if (prefix.length + chunkToSend.length <= MAX_LENGTH) {
        ...
        console.warn(
          `Chunk for ${chatId} cannot have '(lanjutan)' prefix due to length limit.`,
        );
      }
    }
    ...
    try {
      ...
      console.log(
        `Message chunk sent to ${chatId}` +
          (payload.reply_to_message_id
            ? ` in reply to ${payload.reply_to_message_id}`
            : "") +
          ` (length: ${chunkToSend.length})`,
      );
      ...
      console.error(
        `Error sending message chunk to ${chatId}:`,
        error.response
      ...
      if (
        ...
        console.warn(
          `!!! Potential formatting issue in chunk for ${chatId}. Raw text snippet: ${chunkToSend.substring(0, 100)}...`,
        );
        const fallbackText = chunkToSend.replace(/[*_`\[\]()]/g, "");
        try {
          console.log(
            `Attempting fallback send (chunk) without formatting chars to ${chatId}`,
          );
          const fallbackPayload = {
            ...payload,
            text: fallbackText.substring(0, MAX_LENGTH),
          };
          ...
          console.error(
            `Fallback send (chunk) also failed for ${chatId}:`,
            fallbackError.response
              ? JSON.stringify(fallbackError.response.data, null, 2)
              : fallbackError.message,
          );
          console.error(
            `Stopping further message chunks for this message to ${chatId} due to send error.`,
          );
          break;
        }
      } else {
        console.error(
          `Stopping further message chunks for this message to ${chatId} due to non-parsing send error.`,
        );
        break;
      }
    }
...
async function sendPhotoFromBase64(
  chatId,
  base64Data,
  mimeType,
  caption = "",
  ...
  if (!base64Data || !mimeType) {
    console.error(
      `Invalid base64 data or mimeType provided for chat ${chatId}`,
    );
    await sendMessage(
      chatId,
      "Waduh, ada error internal pas mau kirim gambar (data invalid).",
      replyToMessageId,
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `Photo sent successfully to ${chatId}` +
...
function stripMarkdown(text) {
  ...
  for (let i = 0; i < originalLines.length; i++) {
    ...
    if (listItemMatch) {
      ...
      categoryHeaderMatch &&
      !startsWithWhitespaceRegex.test(categoryHeaderMatch[1]) &&
      nextLine &&
      listItemRegex.test(nextLine)
    ) {
...
async function getGeminiResponse(
  ...
  console.log(
    `Using model: ${modelToUse} for chat ${chatId}. Vision request: ${isVisionRequest}`,
  );

  if (history.length === 0) {
    history.push({
      role: "system",
      parts: [{ text: `Pengguna saat ini adalah ${userName}.` }],
    });
  ...
  if (isVisionRequest) {
    ...
    if (!newUserPrompt) {
      ...
      console.log(
        "No text prompt provided with image, using default 'Describe this image.'",
      );
    }
  }
  history.push({ role: "user", parts: currentUserTurnContent });

  const currentHistoryLength = history.reduce(
    (acc, turn) => acc + JSON.stringify(turn).length,
    0,
  );
  ...
  if (
    ...
    currentHistoryLength > MAX_HISTORY_SIZE_BYTES
  ) {
    console.warn(
      `History for chat ${chatId} exceeding limits (Length: ${history.length}/${MAX_HISTORY_LENGTH}, Size: ${currentHistoryLength}/${MAX_HISTORY_SIZE_BYTES}), trimming...`,
    );
    const systemPromptsCount = history.filter(
      (h) => h.role === "system",
    ).length;
    ...
    if (conversationTurns > turnsToKeep * 2) {
      ...
      if (itemsToRemove > 0) {
        ...
        console.log(
          `Trimmed ${itemsToRemove} items (turns) from history for chat ${chatId}`,
        );
      }
    }
  }
  ...
  console.log(
    `Calling Gemini API (${modelToUse}) for chat ${chatId}. User: ${userName}. Prompt: "${newUserPrompt || "(Image only)"}". Grounding: ${enableGrounding}`,
  );
  ...
  if (enableGrounding && !isVisionRequest) {
    ...
    console.warn(
      "Grounding was requested but disabled because this is a vision request.",
    );
    enableGrounding = false;
  }
  ...
  try {
    ...
    if (!candidate) {
      ...
      return {
        text: `Waduh ${userName}, AI-nya nggak ngasih respon nih kali ini. Coba lagi ya.`,
        parseMode: null,
      };
    }
    ...
    if (candidate.finishReason && candidate.finishReason !== "STOP") {
      console.warn(
        `Gemini response finished with reason: ${candidate.finishReason}. Content may be incomplete or blocked.`,
      );
      aiResponseText = candidate.content?.parts?.[0]?.text || "";
      if (!aiResponseText) {
        chatHistories[chatId] = historyBeforeResponse;
        return {
          text: `Maaf ${userName}, respons AI diblokir karena alasan keamanan (${candidate.finishReason}). Coba prompt yang berbeda ya.`,
          parseMode: null,
        };
      }
    ...
    if (aiResponseText) {
      ...
      if (groundingAttributions && groundingAttributions.length > 0) {
        console.log(
          "Grounding attributions found:",
          groundingAttributions.length,
        );
        ...
        const sourceHostnames = groundingAttributions
          .map((source) => {
            try {
              ...
              console.warn(
                `Could not parse URI "${source.uri}" for hostname:`,
                e.message,
              );
              return null;
            }
        ...
        if (sourceHostnames.length > 0) {
          ...
          console.log(
            `Formatted ${sourceHostnames.length} unique source hostnames.`,
          );
        } else {
          console.warn(
            "Grounding was enabled, but no valid hostnames extracted from attributions.",
          );
        }
      } else if (enableGrounding) {
        console.log(
          "Grounding was enabled, but no attributions returned by API.",
        );
      }
      ...
      console.warn(
        "Vision request successful but no text description returned.",
      );
      history.push({
        role: "model",
        parts: [{ text: "(Deskripsi gambar tidak tersedia)" }],
      });
      chatHistories[chatId] = history;
      return {
        text: `Hmm ${userName}, AI-nya bisa lihat gambarnya, tapi nggak bisa ngasih deskripsi teksnya nih. Aneh ya.`,
        parseMode: null,
      };
    } else {
      console.error(
        "Gemini response format unexpected or empty text.",
        JSON.stringify(response.data, null, 2),
      );
      chatHistories[chatId] = historyBeforeResponse;
      return {
        text: "Waduh, AI-nya lagi diem nih, nggak ngasih jawaban.",
        parseMode: null,
      };
    }
  } catch (error) {
    ...
    if (
      ...
      errorMsg = `Aduh ${userName}, kelamaan nih nunggu AI, coba lagi aja`;
    } else if (error.response && error.response.status === 429) {
      errorMsg = `Waduh ${userName}, kebanyakan nanya nih kayaknya, coba santai dulu bentar`;
...
async function generateImageWithGemini(chatId, prompt, userName = "mas") {
  if (!GEMINI_API_KEY) {
    ...
    return {
      error: `Maaf ${userName}, konfigurasi AI untuk gambar belum diatur.`,
    };
  }
  if (!GEMINI_IMAGE_MODEL_NAME) {
    console.error("Gemini Image Model Name is not set.");
    return {
      error: `Maaf ${userName}, model AI untuk gambar belum ditentukan.`,
    };
  }
  if (!prompt || prompt.trim().length === 0) {
    console.log(
      `Image generation skipped for chat ${chatId} due to empty prompt.`,
    );
    return {
      error: `Mau gambar apa, ${userName}? Kasih tau dong. Contoh: /img kucing astronot`,
    };
  }
  ...
  console.log(
    `Calling Gemini Image API (${modelToUse}) for chat ${chatId}. User: ${userName}. Prompt: "${prompt}"`,
  );
  ...
  try {
    ...
    if (!candidate) {
      console.error(
        `Gemini Image response missing candidates for chat ${chatId}.`,
        JSON.stringify(response.data, null, 2),
      );
      return {
        error: `Waduh ${userName}, AI nggak ngasih hasil gambar nih. Coba lagi ya.`,
      };
    }

    if (candidate.finishReason && candidate.finishReason !== "STOP") {
      console.warn(
        `Gemini Image response for chat ${chatId} finished with reason: ${candidate.finishReason}. Checking for partial content.`,
      );
      ...
      if (imagePart?.inlineData?.data) {
        console.log(
          `Image found despite finish reason ${candidate.finishReason} for chat ${chatId}. Proceeding.`,
        );
        return {
        ...
        console.error(
          `Gemini Image generation blocked for chat ${chatId}. Reason: ${candidate.finishReason}`,
        );
        const safetyRatings = candidate.safetyRatings
          ? ` (${candidate.safetyRatings.map((r) => r.category + ":" + r.probability).join(", ")})`
          : "";
        return {
          error: `Waduh ${userName}, gambar mu sus ;-;, generate yang lainnya`,
        };
      }
    }
    ...
    if (imagePart?.inlineData?.data && imagePart?.inlineData?.mimeType) {
      console.log(
        `Image successfully generated for chat ${chatId}. MimeType: ${imagePart.inlineData.mimeType}`,
      );
      const textPart = candidate.content?.parts?.find((part) => part.text);
      const textFallback = textPart ? textPart.text : null;
      ...
      if (textPart?.text) {
        console.warn(
          `Gemini Image API (${modelToUse}) returned text instead of image for chat ${chatId}: "${textPart.text.substring(0, 100)}..."`,
        );
        return { error: `Hmm ${userName}, Gambar mu sus coba ganti prompt` };
      } else {
        console.error(
          `Gemini Image response format unexpected or missing image data for chat ${chatId}.`,
          JSON.stringify(response.data, null, 2),
        );
    ...
    console.error(
      `Error calling Gemini Image API (${modelToUse}) for chat ${chatId}:`,
      error.response
        ? JSON.stringify(error.response.data, null, 2)
        : error.message,
    );
    ...
    if (
      ...
      errorMsg = `Aduh ${userName}, kelamaan nih nunggu AI bikin gambarnya, coba lagi aja`;
    } else if (error.response && error.response.status === 429) {
      errorMsg = `Waduh ${userName}, kebanyakan minta gambar nih kayaknya pake , coba santai dulu bentar`;
    } else if (error.response?.data?.error) {
      const apiError = error.response.data.error;
      ...
      if (apiError.message && apiError.message.includes("API key not valid")) {
        ...
        apiError.message &&
        apiError.message.includes("Request payload size")
      ) {
...
async function handleInlineQuery(inlineQuery, res) {
  ...
  console.log(
    `Received inline query from ${nameForAIContext} (${userId}): "${query}" (ID: ${inlineQueryId})`,
  );
  ...
  for (const [trigger, cmd, grounding] of groundingTriggers) {
    if (lowerCaseQuery.startsWith(trigger)) {
      ...
      console.log(
        `Inline query matched GROUNDING: Command='${command}', Grounding=${enableGrounding}, Trigger='${trigger.trim()}'. Prompt: "${promptForAI}"`,
      );
      break;
    }
  }
  ...
  if (!explicitTriggerFound) {
    ...
    for (const [trigger, cmd, grounding] of chatTriggers) {
      if (lowerCaseQuery.startsWith(trigger)) {
        ...
        explicitTriggerFound = true;
        console.log(
          `Inline query matched EXPLICIT CHAT: Command='${command}', Grounding=${enableGrounding}, Trigger='${trigger.trim()}'. Prompt: "${promptForAI}"`,
        );
        break;
      }
    }
  ...
  if (!explicitTriggerFound && query) {
    ...
    enableGrounding = false;
    console.log(
      `Inline query using DEFAULT CHAT behavior. Command='${command}', Grounding=${enableGrounding}. Prompt: "${promptForAI}"`,
    );
  }
  ...
  if (explicitTriggerFound && !promptForAI) {
    ...
    console.log(
      `Inline query has explicit trigger "${usedTrigger}" but no prompt text.`,
    );
    return answerInlineQuery(
      inlineQueryId,
      [],
      res,
      `Butuh teks setelah ${usedTrigger}...`,
    );
  }
  ...
  try {
    console.log(
      `Getting ${enableGrounding ? "grounded" : "standard"} AI response for inline query ID ${inlineQueryId}...`,
    );
    ...
    if (
      aiResponseObject &&
      aiResponseObject.text &&
      !aiResponseObject.text.toLowerCase().includes("gagal") &&
      !aiResponseObject.text.toLowerCase().includes("maaf")
    ) {
      ...
      console.log(
        `Prepared InlineQueryResultArticle for query ID ${inlineQueryId}`,
      );
    } else {
      errorMessageForResult =
        aiResponseObject.text || `Gagal mendapatkan respons ${command}.`;
      console.warn(
        `AI response failed or contained error for inline query ID ${inlineQueryId}: ${errorMessageForResult}`,
      );
    }
  } catch (error) {
    console.error(
      `Error processing inline query ID ${inlineQueryId} ("${query}"):`,
      error,
    );
    errorMessageForResult =
      "Waduh, ada masalah internal pas proses permintaanmu.";
  }
  ...
  if (errorMessageForResult && results.length === 0) {
    ...
    console.log(
      `Prepared error message as InlineQueryResultArticle for query ID ${inlineQueryId}`,
    );
  }
...
async function answerInlineQuery(
  ...
  try {
    ...
    console.log(
      `Answered inline query ${inlineQueryId} with ${results.length} results.` +
        (switchPmText ? ` Switch PM: "${switchPmText}"` : ""),
    );
    if (!res.headersSent) {
    ...
    console.error(
      `Error answering inline query ${inlineQueryId}:`,
      error.response
        ? JSON.stringify(error.response.data, null, 2)
        : error.message,
    );
...
async function generateImageForInlineQuery(prompt) {
  try {
    ...
    if (imageResult.error) {
      console.warn(
        `Image generation failed for inline query "${prompt}": ${imageResult.error}`,
      );
    ...
    if (imageResult.base64Data && imageResult.mimeType) {
      ...
      console.error(
        `Unexpected result from generateImageWithGemini for inline query "${prompt}"`,
      );
...
async function answerInlineQuery(
  ...
  res,
  switchPmText = null,
  switchPmParameter = "inline_error",
) {
  const payload = {
    inline_query_id: inlineQueryId,
    results: results,
...
module.exports = async (req, res) => {
  ...
  if (update.message && update.message.chat && update.message.from) {
    ...
    const nameForAIContext =
      ...
      (username ? `@${username}` : null) ||
      `User_${userId}`;
    const nameForBotGreeting =
      nickname || firstName || (username ? `@${username}` : null) || "kamu";
    ...
    const chatTriggers = ["/chat ", "maomao ", "mao "];
    ...
    for (const trigger of imageTriggers) {
      if (lowerCaseText.startsWith(trigger)) {
        ...
        if (promptForAI) {
          ...
          console.log(
            `Processing IMAGE generation request (Trigger: '${triggerWordUsed}') from ${nameForAIContext} (${userId})`,
          );
        } else {
          await sendMessage(
            chatId,
            `Mau ${triggerWordUsed} apa, ${nameForBotGreeting}? Kasih tau dong. Contoh: ${triggerWordUsed} pemandangan senja di pantai`,
            messageIdToReply,
          );
    ...
    if (!shouldGenerateImage) {
      if (lowerCaseText === "/clear") {
        if (chatHistories[chatId]) {
          ...
          await sendMessage(
            chatId,
            `Oke ${nameForBotGreeting}, history obrolan sudah dibersihkan!`,
            messageIdToReply,
          );
          console.log(
            `History cleared for chat ${chatId} by ${nameForAIContext} (${userId})`,
          );
        } else {
          await sendMessage(
            chatId,
            `Hmm ${nameForBotGreeting}, belum ada history buat dihapus.`,
            messageIdToReply,
          );
        }
      ...
      if (repliedToMessage?.photo?.length > 0 && messageText) {
        console.log(
          `Detected reply to photo by ${nameForAIContext} (${userId}). Checking text trigger...`,
        );
        ...
        for (const trigger of chatTriggers) {
          if (lowerCaseText.startsWith(trigger)) {
            ...
            promptForAI = messageText.substring(trigger.length).trim();
            console.log(
              `Vision trigger '${trigger.trim()}' found in reply text.`,
            );
            ...
            try {
              ...
              const photo =
                repliedToMessage.photo[repliedToMessage.photo.length - 1];
              ...
              console.log(
                `Image downloaded (${((imageBase64.length * 3) / 4 / 1024).toFixed(2)} KB) and encoded. MimeType: ${imageMimeType}`,
              );
              shouldProcessAI = true;
              enableGrounding = false;
            } catch (error) {
              console.error(
                `Error fetching/processing image for vision request (file_id: ${fileId}):`,
                error.message,
              );
              await sendMessage(
                chatId,
                `Waduh ${nameForBotGreeting}, gagal ngambil/proses gambarnya nih. Coba lagi ya.`,
                messageId,
              );
        ...
        if (!visionTriggerFound && messageText) {
          console.log(
            `Ignoring reply to photo from ${nameForAIContext} (${userId}) because text does not start with a valid chat trigger.`,
          );
        }
      }
        ...
        for (const trigger of groundingTriggers) {
          if (lowerCaseText.startsWith(trigger)) {
            ...
            if (promptForAI) {
              ...
              console.log(
                `Processing TEXT message ${messageId} WITH grounding (Trigger: '${triggerWordUsed}') from ${nameForAIContext} (${userId})`,
              );
            } else {
              await sendMessage(
                chatId,
                `Iya ${nameForBotGreeting}, mau cari ${triggerWordUsed} apa? Contoh: ${triggerWordUsed} berita terkini tentang AI`,
                messageIdToReply,
              );
              shouldProcessAI = false;
            }
        ...
        if (!groundingTriggerFound && !shouldProcessAI) {
          if (chatType === "private") {
            if (messageText) {
              ...
              enableGrounding = false;
              if (
                lowerCaseText.startsWith("cari info ") ||
                lowerCaseText.startsWith("inpo ")
              ) {
                const query = messageText
                  .substring(messageText.indexOf(" ") + 1)
                  .trim();
                if (query) {
                  promptForAI = query;
                  enableGrounding = true;
                  ...
                  console.log(
                    `Processing private message ${messageId} WITH grounding (Implicit trigger) from ${nameForAIContext} (${userId})`,
                  );
                }
              } else {
                console.log(
                  `Processing private message ${messageId} (no grounding) from ${nameForAIContext} (${userId})`,
                );
              }
            } else {
              console.log(
                `Ignoring empty private message ${messageId} from ${nameForAIContext} (${userId})`,
              );
              shouldProcessAI = false;
            }
            ...
            if (
              !textTriggerFound &&
              BOT_USER_ID &&
              repliedToMessage?.from?.id === BOT_USER_ID &&
              ...
              if (!repliedToMessage.photo) {
                ...
                let history = chatHistories[chatId] || [];
                const lastBotTurnIndex = history
                  .map((h) => h.role)
                  .lastIndexOf("model");

                if (
                  lastBotTurnIndex !== -1 &&
                  history[lastBotTurnIndex].parts[0].text.includes(
                    botPreviousText.substring(0, 50),
                  )
                ) {
                  promptForAI = userReplyText;
                  console.log(
                    `Continuing conversation based on reply to bot message ${repliedToMessage.message_id}`,
                  );
                } else {
                  console.warn(
                    `Could not find matching bot turn in history for reply ${repliedToMessage.message_id}. Creating manual context.`,
                  );
                  promptForAI = `Ini adalah respons saya sebelumnya: "${botPreviousText}"\n\nSekarang tanggapi ini dari ${nameForAIContext}: "${userReplyText}"`;
                  if (history.length > 2) {
                    const systemPrompts = history.filter(
                      (h) => h.role === "system",
                    );
                    chatHistories[chatId] = systemPrompts;
                    console.warn(
                      `Resetting history for chat ${chatId} due to potential context mismatch.`,
                    );
                  }
                }
                textTriggerFound = true;
            ...
            if (
              ...
              if (repliedFrom) {
                const repliedUsername = repliedFrom.username
                  ? repliedFrom.username.toLowerCase()
                  : null;
                const repliedNickname = repliedUsername
                  ? userNicknames[repliedUsername]
                  : null;
                originalSenderName =
                  repliedNickname ||
                  repliedFrom.first_name ||
              ...
              console.log(
                `Added context from replied text message ${repliedToMessage.message_id}`,
              );
            ...
            if (textTriggerFound && promptForAI) {
              ...
              enableGrounding = false;
              console.log(
                `Trigger TEXT '${triggerWordUsed}' activated (no grounding) for message ${messageId} in group ${chatId} by ${nameForAIContext} (${userId})`,
              );
            } else if (
              textTriggerFound &&
              !promptForAI &&
              ...
              if (!(repliedToMessage && repliedToMessage.photo)) {
                console.log(
                  `Ignoring non-trigger text message ${messageId} in group chat ${chatId} from ${nameForAIContext} (${userId})`,
                );
              }
              shouldProcessAI = false;
            }
            ...
            console.log(
              `Ignoring message ${messageId} in chat ${chatId} because it has no text content and is not a reply to a photo.`,
            );
            shouldProcessAI = false;
          } else {
            if (!(repliedToMessage && repliedToMessage.photo)) {
              console.log(
                `Ignoring message from unsupported chat type: ${chatType} or unhandled condition.`,
              );
            }
            shouldProcessAI = false;
          }
    ...
    if (shouldProcessAI) {
      const effectivePromptLength =
        (promptForAI || "").length + (imageBase64 ? imageBase64.length : 0);
      const MAX_EFFECTIVE_PROMPT = 4 * 1024 * 1024;

      console.log(
        `Effective TEXT/VISION prompt/image size: ${effectivePromptLength} bytes (Limit: ${MAX_EFFECTIVE_PROMPT})`,
      );

      if (effectivePromptLength > MAX_EFFECTIVE_PROMPT) {
        await sendMessage(
          chatId,
          `Waduh ${nameForBotGreeting}, permintaannya (${triggerWordUsed}) terlalu besar nih (prompt/gambar > ${(MAX_EFFECTIVE_PROMPT / 1024 / 1024).toFixed(1)} MB). Coba dipersingkat atau pakai gambar lebih kecil ya.`,
          messageIdToReply,
        );
      } else if (!promptForAI && !imageBase64) {
        console.warn(
          `shouldProcessAI is true but both prompt and image are missing for chat ${chatId}, message ${messageId}. Skipping.`,
        );
      } else {
        if (!imageBase64) {
          try {
            ...
            console.warn("Could not send typing action:", actionError.message);
          }
        }
      ...
      try {
        ...
        console.warn(
          "Could not send upload_photo action:",
          actionError.message,
        );
      }
      ...
      if (imageResult.base64Data && imageResult.mimeType) {
        ...
        await sendMessage(
          chatId,
          imageResult.error ||
            `Waduh ${nameForBotGreeting}, gagal bikin gambarnya nih, coba lagi nanti ya.`,
          messageIdToReply,
        );
      }
    }
    ...
    const chatId = update.message.chat.id;
    console.log(
      `Ignoring non-text/photo/incomplete message update in chat ${chatId || "unknown"}`,
    );
  } else {
    console.log(
      "Ignoring update that is not a message or lacks required fields.",
    );
  }
